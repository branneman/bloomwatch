...
### 911 — Deepen mana curve, consumable throughput, and Innervate calibration ✅ Done
...
**Findings:** the whole local `calibration-data/` corpus (100 reports) was regenerated fresh via `scripts/calibrate.ts` first, per this story's own acceptance criteria. Tabulated against the story-901 20-report deep-resto exemplar corpus (kills only, duration filters per each metric's own existing rule), corroborated against the broader 100-report talent-tagged corpus:

- **Ending mana %: no change.** 143 judged kill-fights ≥90s in the exemplar corpus: good 40% / fair 38% / bad 22%, median 34.5%, p25 14.9%, p75 62.5%, p90 88.5% (1 fight excluded — a known `classResources`-shape misdetection artifact producing an out-of-range percentage, not itself in scope here). The current good band (5-40%) contains the real median almost exactly, and p75 (62.5%) sits just under the 70% bad boundary — a solid natural fit, not the over-harsh "convicts the median" pattern found broken elsewhere. The broader corpus (641 fights) lands within 3 points of every figure above, confirming this isn't exemplar-corpus noise.
- **Consumable throughput — Mana Potion: no change.** 210 mana-constrained fights: good 59% / fair 28% / bad 13%, used-vs-floor delta median 0 (real elite players meet the 1-per-120s floor almost exactly on the median fight). This is a well-fit threshold.
- **Consumable throughput — Rune: floor interval widened from 120s to 300s.** Real elite play showed a Rune (Dark Rune/Demonic Rune, shared cooldown) used in only 21/210 (10%) of mana-constrained exemplar fights regardless of length — mean use stayed at 0.03-0.32 even across 4-8 minute fights, never scaling with duration the way Mana Potion genuinely does. Under the old shared 120s floor, real elite players scored good only 20% of the time (32% bad) — median real usage was 0 in every fight-length bucket, the same "convicts the median real player" pattern 909 found broken in Swiftmend's old wasteful-share band. This tracks with real TBC context: Dark Rune requires Enchanting-crafted reagents and Demonic Rune is a scarce/limited vendor item, both cost health to use, unlike Mana Potions which are cheap and always stackable — a materially different real-world availability, not a process failing. `RUNE_FLOOR_INTERVAL_MS` (now split from Mana Potion's own `POTION_FLOOR_INTERVAL_MS`, unchanged at 120s) moved to 300s, which fits the real data much better: good 80% / fair 20% / bad 1% on the same exemplar corpus (good 86% / fair 14% / bad 0% on the broader corpus), while still flagging genuine neglect in unusually long fights.
- **Innervate audit: no change**, with one flagged (not fixed) caveat. 160 judged-relevant fights in the exemplar corpus: good 75% / fair 6% / bad 19%. Breaking down by branch: the ally-cast "wasted on a non-mana-using target" bad path never fired once in real data (0/28 ally casts) — a real, if rarely-exercised, safety net, the same "structurally sound, no change" shape as 909's downranking-discipline finding. The self-cast timing split (good if before the fight's final 10%, fair if late) landed 92/102 (90%) on-time vs. 10/102 (10%) late — a sensible, non-degenerate spread. The entire "bad" bucket (30/160, matching the overall 19% bad rate exactly) comes from the "no Innervate cast in a mana-constrained fight ≥3 min" branch. **Flagged, not fixed:** 23 of those 30 cases (77%) cluster within fight durations of 3.0-4.4 minutes — uncomfortably close to half of Innervate's real 6-minute TBC cooldown, and plausibly explained by the ability still being on cooldown from the previous pull rather than a genuine miss. `computeInnervateAudit` has no cross-fight cooldown tracking at all (unlike `deathForensics.ts`'s `isReady`, which checks readiness within a single fight); fixing this properly needs tracking a druid's Innervate casts across their whole report history, which is a real logic addition, not a threshold-value move, so it's out of this story's scope — the same boundary 909 drew around Swiftmend's `classify()` logic. Raising the 3-minute duration cutoff alone wouldn't actually fix this (a longer fight can still have Innervate on cooldown the whole time from a cast just before pull), so no threshold value was changed here; the 19%/11% bad rates in the two corpora should be read with this caveat until a future story adds cross-fight readiness tracking.
...
### 914 — Revisit metrics currently left un-judged (informational-only) ✅ Done
...
**Resolved 2026-07-21:** added a real judgement, reusing Rejuvenation's own good<5%/fair5-15%/bad>15% bands, for every archetype bucket except deep-resto — every non-deep-resto bucket is confirmed Restoration <41 by construction (`classifyBucket`), so the Tree of Life exemption's premise never held for them, and pooling the local calibration corpus found those same bands fit non-deep-resto play cleanly (69% good/9% fair/22% bad, non-degenerate). `unknown-no-talent-data` (failed talent read) is grouped with deep-resto instead — a failed read can't rule out the exemption applying, matching story 903d's identical reasoning. Wired through `hotClipDetection.ts`, `HotClipDetectionCard`, `useSpellDisciplineSummary`, and `scripts/lib/calibrateReport.ts`. See `docs/thresholds.md`'s Spell discipline section for the full numeric findings.
...
**Resolved 2026-07-21: confirmed, stays informational.** Pooled Rejuvenation/Regrowth-HoT overheal against the full local corpus (`calibration-data/`, 120 reports, tagged via `docs/calibration-archetypes.json`) and it does _not_ cluster the way Bloom overheal did — deep-resto median 54%/57%, dreamstate median 61%/64%, with a real p10-p90 spread of 35-45+ points per archetype bucket, nothing like Bloom's tight 72-74%. The decisive check: for druids with 3+ fights, average within-player fight-to-fight spread (41/50 points for Rejuvenation/Regrowth-HoT) exceeds average between-player spread (27/31 points) — the same druid swings more across their own fights than typical players differ from each other. This held even after adding 20 reports from a second guild's self-labeled elite deep-resto healer (high WCL parses, consistent 0/0/61 deep-resto talents): that player's own Rejuvenation overheal ranged 19-84% within a single 13-fight report. That's the signature of an encounter-driven number (movement, add phases, how much raid-wide healing overlaps a given pull) rather than a stable per-player skill trait, unlike Bloom overheal's genuine skill signal — the original "inherent, don't judge" call is confirmed as-is, no threshold added. See `docs/thresholds.md`'s mana economy section for the full numeric findings.

With all four bullets above resolved, story 914 is done.
...
### 915 — Resolve unknown pre-pull Lifebloom stack state via bounded event lookback ✅ Done

I want a target whose Lifebloom was already active before the fetched event window begins (pre-pull stacking, or a WCL fight-ID split across a wipe) to have its real 3-stack history resolved where possible, instead of confidently reading as "0%, bad" when the druid may well have had it fully stacked the whole time. Found directly while verifying story 201/205's per-target judgement against a real report: `DRtXV4ChA2Kw3c81` fight 84 ("The Illidari Council"), druid Stuuri, shows targets Jeloviina (id 30) and Hulina (id 6) at `lb3UptimePct: 0`, `lbUptimePct: 100` (100% any-stack uptime, i.e. Lifebloom was present the _entire_ 240s fight), `windowMs` equal to the full fight duration. Pulling the raw Buffs events for both targets confirmed every single event within the fight window is a solo `refreshbuff` (no `applybuff`, no `applybuffstack`) at a consistent ~6.5s cadence — identical to Stuuri's own reported "good" 6.5s median refresh cadence on the primary tank target — for the entire fight. That combination (buff already active at fight start, refreshed on a disciplined cadence, never once showing a fresh application) is strong evidence the target was genuinely maintained (very plausibly at 3 stacks) before the pull, not neglected.

The root cause is `lifebloomStacks.ts`'s `deriveLifebloomTargetState` (lines ~125-136): when a target's timeline opens with something other than `"open"`, it deliberately (and correctly, for the case it was built for — a wipe splitting one pull across two WCL fight IDs) infers any-stack presence back to `fightStart` but refuses to assume the pre-existing stack was 3, since "the exact prior stack count is unknown." That conservatism is reasonable as a _default_, but currently has no path to becoming more certain — it always resolves to "unknown" being treated identically to "confirmed never reached 3 stacks," which is a real, confident false negative.

**Proposed mechanic** (agreed directly, not yet designed in detail): when a target's fight-window timeline is detected to open with a carry-in signal (first event kind ≠ `"open"`), fetch **Buffs** events (not Casts — stack count only exists on `applybuffstack`'s `stack` field) for the 60 seconds immediately before `fightStart`, merge them with the normal fight-window events, and re-derive the target's true carried-in stack state from that combined timeline. If a genuine `"open"`/`applybuffstack` sequence is found in the lookback window, resolve the real stack count and continue normal processing from there. If the ambiguity persists even after the 60s lookback (verified against the motivating example above: tracing target 30's actual history back further than 60s — over 190 seconds — never surfaced an `applybuff` under Stuuri's own source ID at all, suggesting either an even earlier application or a hand-off from a different druid entirely), fall back to excluding that target from judgement entirely (treated as unjudgeable/unknown, not "bad") rather than reporting a false negative.

**This is an architecture-level change, not a threshold tweak**: `computeLb3Uptime` and `computeConcurrentLb3Targets` (both consumers of `reconstructLifebloomTimelines`/`deriveLifebloomTargetState`) are currently pure, synchronous functions — a documented, deliberate convention shared by every `compute*` function in `src/metrics/*.ts` so both the live UI and `scripts/calibrate.ts` can consume them identically. A conditional "detect ambiguity → fetch more → recompute" flow needs the _caller_ (each React hook — `useLifebloomDisciplineSummary`, the concurrent-targets fetch path, `ConcurrentTargetsCard`, `LB3UptimeCard` — plus `scripts/lib/calibrateReport.ts`) to orchestrate a second, conditional fetch; the pure compute layer should only gain a way to accept already-merged lookback data, not fetch anything itself.

**Acceptance criteria**

- A new pure helper detects, from already-fetched fight-window events alone, which target IDs show the carry-in signal (timeline's first event kind ≠ `"open"`) and would benefit from a lookback fetch.
- `deriveLifebloomTargetState` (or a new function alongside it) can consume a timeline built from `[fightStart - 60_000, fightEnd]` events and correctly resolve the true stack-3 state established before `fightStart`, rather than assuming unknown.
- Each calling site (both hooks and `calibrateReport.ts`) performs the second Buffs-events fetch only when the detector flags at least one ambiguous target for that fight — never unconditionally, to avoid padding out WCL request counts the way story 010's audit specifically worked to eliminate.
...
**Dependency note added while scoping story 917's phase 2 (Faerie Fire duty mitigation):** story 917's own empirical study found that genuine Faerie Fire duty measurably drags `computeRestackTax`'s current cast-count-based judgement — a real, within-druid-paired effect (median +3 extra casts per fight, ~69% of paired druids showing the same direction) that survives duration normalization (bad-rate 70% FF-duty vs 53% non-FF-duty across the local corpus) — the only one of 5 candidate metrics 917 checked that held up once compared per-druid rather than pooled across druids. 917's phase 2 ships a mitigation against `judgeRestackTax`'s _current_ cast-count formula: a flat allowance added to both `goodMax`/`fairMax` when the druid is detected on FF duty that fight, calibrated against the real corpus (using `judgeThresholdBelow`'s exact `<`/`<=` boundary semantics, not an approximation) to **+5 casts** (the value that brought FF-duty's good/fair/bad distribution closest to the non-FF-duty baseline: good 45.2% vs baseline 44.4%, bad 52.2% vs baseline 53.3%). **This mitigation is built against the soon-to-be-superseded cast-count unit, not the "stack-loss event" unit this story plans to introduce** — whoever picks up 916 needs to re-derive an equivalent FF-duty allowance for the new rebuild-event-based headline number rather than assuming the old +5-cast constant transfers unchanged, and should re-run the same within-druid comparison this story's phase 2 used (not just recalibrate the exemplar corpus against the new unit in isolation) to confirm the new unit still shows the same real effect before carrying the mitigation forward.

### 917 — Detect and mitigate Faerie Fire raid-assignment drag on Balance-leaning healers 🔲 Todo

I want a Balance-leaning healer (Dreamstate, Restokin, or mostly-balance, per story 900/903a's talent buckets) who is genuinely assigned to maintain Faerie Fire on the boss — a real raid-leader call when the roster has no dedicated Boomkin — to stop having that assignment read as worse healing process than a druid without it. Reported directly: casting Faerie Fire on its own ~40s refresh cadence is a legitimate, raid-requested GCD cost, not a rotation mistake, and shouldn't silently tank any metric that implicitly assumes every one of the druid's GCDs goes to healing.

**Findings needed before implementation** (the reporter's own proposed method — check whether a ~40s-cadence Faerie Fire cast would tip any metric's score, then mitigate only the ones it actually tips, rather than assuming which ones up front):

- `computeCastIntervals` (`src/metrics/castIntervals.ts`), which both GCD utilization (101) and idle-gap detection (102) are built on, counts _any_ of the druid's casts by `sourceID` alone — a Faerie Fire cast already occupies a GCD interval exactly like a heal does, so on inspection those two metrics are not the risk this story's originating report first assumed. Confirm that with real data before spending design effort mitigating a problem that may not exist there.
- The real risk is more likely in metrics that assume a GCD not spent on a tracked healing spell was wasted, or that penalize a delayed healing decision an FF cast could cause: LB3 refresh cadence (202, a late refresh past the "late" band), accidental blooms (203, a stack allowed to bloom because the GCD went to FF instead of a timely refresh), re-stack tax (204, the resulting rebuild cost), and mana economy (401/402, mana spent on FF with no healing return). Each is a candidate to check empirically against the local calibration corpus (`calibration-data/`), not assumed true.
- Downranking discipline (303), overheal (404), and prep hygiene (601/602) aren't expected to interact with FF cadence at all and don't need investigating unless the corpus check above says otherwise.

**Acceptance criteria**

- Faerie Fire (all ranks — TBC has multiple, per story 007's per-report resolution convention) is resolved from the report's own `masterData.abilities` at runtime, never hardcoded, following story 906's locale-independent game-ID-first pattern; it does not belong in `resolveAbilities.ts`'s `DruidHealingSpell` union (Faerie Fire is a debuff, not a heal) and gets its own resolution path.
- A behavioral detector identifies "on FF duty" for a fight: the selected druid is in a Balance-leaning talent bucket (story 900/903a: `likely-dreamstate-full`, `likely-dreamstate-partial`, `mostly-balance`, or `restokin-shaped`) AND casts Faerie Fire on a boss-tier enemy at a sustained cadence for a meaningful share of the fight — not a one-off pull-opener cast, which any archetype might do incidentally. "Boss-tier" is WCL's own `subType: "Boss"` on the report's NPC actors (confirmed live: council fights like Fathom-Lord Karathress and High King Maulgar carry multiple simultaneous `Boss`-tagged actors, not one) — trash/adds spawned mid-encounter are never boss-tier and don't count, matching this app's existing trash-exclusion convention (story 013). Only Faerie Fire proper counts, never Faerie Fire (Feral) — a separate, lesser ability Feral druids get for free in shapeshifted form, distinct from the Improved Faerie Fire talent modifying the real spell.
- Each metric candidate listed above is checked against the local calibration corpus: compare FF-duty fights against non-FF-duty fights of the same archetype bucket and similar skill level, and report per-metric whether FF duty measurably drags the score. A metric that shows no real effect is recorded as "checked, no mitigation needed" (matching this repo's existing "no change" calibration-finding convention, e.g. stories 908/909/911) rather than silently skipped.
- For every metric confirmed to be measurably dragged by real FF duty, a mitigation is designed and implemented — the exact mechanism (e.g. excluding sanctioned FF-displaced GCDs from the relevant computation, a per-archetype threshold adjustment in the style of story 905, or something else) is an implementation-time design decision, not predetermined here.
- The mitigation only ever helps a druid genuinely on FF duty; it must not soften judgement for a Balance-leaning druid who casts Faerie Fire occasionally with no real assignment behind it, or for a deep-resto druid casting it at all (which remains a real rotation question, not a sanctioned utility duty, for that archetype).
- `docs/thresholds.md` gains a dated section documenting which metrics were checked, which showed a real effect, and what mitigation (if any) was applied — same convention as every other Epic I calibration finding.

**Findings so far (2026-07-23), from a real corpus check via `npm run analyze:ff-drag`:** 44 reports, regenerated fresh via `npm run calibrate` for every Balance-leaning-bucket (`likely-dreamstate-full`/`likely-dreamstate-partial`/`mostly-balance`) report:druid pair in `docs/calibration-archetypes.json`, filtered to real healing participation (≥20 healing casts/fight average — excludes known non-healer Boomkin alts like Serpentx/Toxickn found during this story's scoping). **Caveat:** the local corpus currently contains zero `likely-dreamstate-partial`/`mostly-balance` entries, so every number below reflects `likely-dreamstate-full` druids only — the detector itself works for all three buckets, but this run's findings haven't been validated against the other two. **A second, more fundamental caveat: this comparison pools every qualifying druid's fights into two global groups (`ffDuty`/`nonFfDuty`) and compares aggregate medians, rather than pairing each druid against their own baseline** — the design spec's promised "within-druid, within-bucket comparison (controls for skill and build)" was not actually implemented, so a baseline skill or roster difference between the two groups (dominated by different, mostly non-overlapping sets of druids) is indistinguishable from a true FF-duty effect. The findings below are real associations worth acting on, but a future within-druid comparison would be more rigorous and should be done before phase 2 commits to a mitigation magnitude.

- **Re-stack tax (204): real, substantial drag, needs mitigation in phase 2.** FF-duty fights median 9 casts (n=228) vs. non-FF-duty median 3 (n=45) — roughly triples. `judgeRestackTax`'s bands scale with fight duration (goodMax = floor(minutes/2)+1, fairMax = floor(minutes)), so for a typical multi-minute boss fight a jump from 3 to 9 plausibly crosses from good/fair into fair/bad. This is the clearest, most actionable finding: FF duty measurably drags this metric. **Caveat:** the comparison above is on raw cast count, not `judgeRestackTax`'s own duration-normalized judgement, so part of this gap could reflect FF-duty fights simply running longer on average rather than a pure FF-duty effect; phase 2 should re-check against the judged bands (not just raw counts) before committing to a mitigation magnitude.
- **Accidental blooms (203): a real but modest effect.** FF-duty median 2 (n=228) vs. non-FF-duty median 1 (n=45) — both medians land in the same fair band (good 0 / fair 1–2 / bad ≥3), just at opposite ends of it. Worth a look in phase 2, but a smaller effect than re-stack tax.
- **LB3 refresh cadence (202): a small, real difference that doesn't clearly point toward drag.** FF-duty median 5428.5ms (n=180) vs. non-FF-duty median 5027ms (n=29) — both land in the same fair band (6–7s good / 5–6s fair / <5s or >7s bad). The non-FF-duty median sits right at the edge of the bad boundary (<5s), while the FF-duty median sits more comfortably mid-fair — if anything the difference doesn't clearly point toward FF duty being the worse case here. Worth a second look with more non-FF-duty data rather than a confident finding either way.
- **Ending mana % (401): no meaningful drag in the direction this story worried about; this metric's own design actually cuts the other way — checked, no mitigation needed.** FF-duty median 41.587...% (n=228) vs. non-FF-duty median 47.465...% (n=43). Both land in `judgeManaBand`'s fair 40–70% band, but FF-duty's is lower, closer to the 5–40% good boundary that rewards spending mana rather than hoarding it (the only bad zone is >70%, hoarding). Mana spent on Faerie Fire doesn't push this metric toward a worse judgement.
- **Consumable throughput (402): no measurable difference — checked, no mitigation needed.** Used-minus-floor delta median is 0 for both FF-duty (n=228) and non-FF-duty (n=45) fights. Caveat: this may partly reflect that many fights in this corpus have `expectedFloor: 0` (short fights requiring no consumables at all), so it's a real "no change" finding rather than a definitive rule-out of any mana-pressure effect specific to this metric.

Story 917 remains 🔲 Todo — these findings satisfy only phase 1's empirical-check acceptance criteria above; designing and implementing a mitigation for re-stack tax (and, to a lesser degree, accidental blooms) is phase 2, a separate future spec/plan not built yet.

### 918 — Faerie Fire boss uptime metric for druids on FF duty 🔲 Todo

I want my Faerie Fire uptime on every boss-tier enemy in an encounter measured and credited when I'm genuinely on FF duty (per story 917's detector), so that a raid-requested assignment that costs real healing throughput is recognized as a positive contribution instead of staying invisible. Surfaced directly while investigating 917's corpus data: council fights (Fathom-Lord Karathress + 3 Fathom-Guards, High King Maulgar + 4 lieutenants) carry multiple simultaneous boss-tier enemies (WCL's `subType: "Boss"`) — a druid on FF duty for a council fight needs to keep the debuff rolling on _all_ of them, not just one, which a single combined cast-cadence number can't represent (mixing casts across several simultaneous targets collapses the apparent interval to a few seconds, looking nothing like a real ~40s refresh rhythm).

**Corpus reality check, confirmed live while scoping this story:** real per-target Debuff-uptime tracking (not just cast counts) on the 4 highest-cast-count Fathom-Lord Karathress fights in the local corpus shows the _same_ pattern every time — the main boss (Karathress) gets well-maintained uptime (85-94%), but all three Fathom-Guards are consistently neglected (16-50% uptime each). No example in the corpus comes close to 95% sustained across all 4 simultaneous targets in a council fight; real players uniformly prioritize the boss and let the adds slide. A genuinely excellent single-boss example does exist (`gNYhK1ZAP7RQz2pa` fight 18, Morogrim Tidewalker, druid Cowpop: 98.5% uptime, refreshed every ~34-39s for the whole 199s fight, no gaps) — so the mechanism itself works and single-target excellence is achievable and real, but this story's eventual good/fair/bad bands for a _council_ fight must be calibrated against what real skilled play actually achieves there, not an assumed uniform 95%+ bar that nobody in the corpus reaches.

**Acceptance criteria**

- Only computed for fights where story 917's FF-duty detector identifies the druid as on FF duty that fight; a Balance-leaning druid who casts Faerie Fire incidentally (no real assignment) isn't judged by this metric, and a deep-resto druid casting it at all isn't either (matches 917's own scoping).
- Per-target Faerie Fire uptime (debuff-active %, the same style of calculation Lifebloom's own uptime tracking already does in `lifebloomStacks.ts`) is tracked independently for every boss-tier enemy present in the fight, not one combined timeline — a council fight's score reflects uptime across all of them, not whichever one happened to get targeted most.
- The fight's overall judgement rewards high uptime sustained across every boss-tier target in the encounter; a strong single-target uptime that ignores other simultaneous boss-tier enemies in a council fight should not read as good.
- Purely a credit/reward metric, not a punitive one — a druid not on FF duty is simply unjudged here, never marked down; 917 already separately handles keeping FF duty from silently dragging a druid's other, unrelated healing-process scores.
- New card (or an extension of wherever 917 surfaces "on FF duty" context), placement and exact good/fair/bad bands are implementation-time decisions — thresholds ship provisional pending a calibration pass against the local corpus, matching this project's usual convention for a brand-new metric with no existing exemplar data.
- `docs/thresholds.md` gains a new entry for this metric once implemented, same convention as every other judged metric.
...
**Note on current behavior, checked against the real shipped code before writing this story:** story 1001 already ships a real per-crisis judgement, not context-only informational output as this story was first reported. `computeNearDeathResponse` (`src/metrics/nearDeathResponse.ts`) already judges every crisis on a maintained target (or every crisis at all, when the druid has no clear 1-2 target tank assignment) `"good"` if any new reactive cast from `HEALING_SPELLS_FOR_RESPONSE` (Lifebloom, Rejuvenation, Regrowth, Healing Touch, Swiftmend, Tranquility) landed on the target during the crisis window, otherwise `judgeDeathReadiness(unspentCount)` off the same Swiftmend-ready/NS-ready/idle-preceding tally story 501 already uses for deaths (0 unspent -> good, 1 -> fair, 2+ -> bad) — which already produces "bad" for exactly the reported "maintained target, Swiftmend ready, NS ready, no reactive heal" case, since that's 2 unspent resources. What's genuinely missing against the fuller ask below is (a) any distinction between a routine single heal landing and a clearly-deliberate burst save, and (b) a judged tier for a crisis outside the maintained-target assignment, which today falls all the way to context-only.

**Acceptance criteria**

- "Good" gains a stronger, distinguishable tier for a clearly deliberate save — at minimum, a Nature's Swiftness cast immediately followed by Healing Touch or Regrowth, or a Swiftmend landing as the reactive cast (consuming a Rejuvenation HoT), recognized as an unambiguous "you clutched this," distinct from any other single reactive heal landing (which still counts as responded per 1001's existing logic — this story adds a distinction within "good," it doesn't remove the existing broader definition of "responded"). The exact list of what else counts as a "clear save" combo, beyond the three named above, is confirmed against real crisis data in the local calibration corpus before shipping, not assumed from the three motivating examples alone.
- A new judged tier ("fair") covers a crisis on a target outside the druid's maintained-target assignment (currently excluded from judgement entirely by `hasClearAssignment` in `computeNearDeathResponse`) when both Swiftmend and Nature's Swiftness were ready — resources were available and could have helped, even though the target wasn't "yours." A crisis on an unmaintained target with neither resource ready remains context-only/unjudged, same as today.
