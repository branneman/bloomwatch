...
**Note (flagged by 011, not fixed here):** `deathForensics.ts`'s `isReady()` helper (used for both `swiftmendReady` and `nsReady`) treats "no prior cast of this ability at all" as "ready" — correct for a druid who simply hasn't needed the cooldown yet, but indistinguishable from a druid whose build can never talent into it at all (e.g. a Dreamstate build, which tops out at 26 Restoration points — well short of Swiftmend, and shallower still of Tree of Life, Restoration's actual 41-point capstone — see `docs/testing.md`'s `bKRZ68XqgwYkxtzm` entry, whose real captured data confirms zero Swiftmend casts across every fight for this exact build). A Dreamstate druid's `unspentCount` is therefore inflated by a resource they never had access to, which can misjudge a death as red for "2 unspent resources" when only Nature's Swiftness was ever real. Fixing this needs the app to resolve a druid's actual talent points, which it doesn't do anywhere today — out of scope for 011's fixture-only mandate. A follow-up story should add talent-point resolution and gate CD-readiness checks on it.
