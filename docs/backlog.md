...
### 917 — Detect and mitigate Faerie Fire raid-assignment drag on Balance-leaning healers ✅ Done
...
**Findings (2026-07-23, refined and confirmed), from a real corpus check via `npm run analyze:ff-drag`:** 44 reports, regenerated fresh via `npm run calibrate` for every Balance-leaning-bucket (`likely-dreamstate-full`/`likely-dreamstate-partial`/`mostly-balance`) report:druid pair in `docs/calibration-archetypes.json`, filtered to real healing participation (≥20 healing casts/fight average — excludes known non-healer Boomkin alts like Serpentx/Toxickn found during this story's scoping). **Caveat:** the local corpus currently contains zero `likely-dreamstate-partial`/`mostly-balance` entries, so every number below reflects `likely-dreamstate-full` druids only — the detector itself works for all three buckets, but this run's findings haven't been validated against the other two. An earlier same-day pass's pooled-only comparison (which couldn't isolate a real per-druid effect from a cross-druid confound) has been superseded by a within-druid paired comparison (27 druid-report pairs with fights in both groups) alongside the real, duration-normalized `judgement` field for each metric.

- **Re-stack tax: real, robust drag — mitigated.** Within-druid paired median delta +3 casts (18 of 27 positive), and duration-normalized bad-rate 70% (FF-duty, n=228) vs. 53% (non-FF-duty, n=45) — a genuine ~17-percentage-point gap that survives both the pooling fix and the raw-count-vs-judgement fix. This is the only metric confirmed measurably dragged. Mitigated: `judgeRestackTax` (`src/metrics/restackTax.ts`) widens `goodMax`/`fairMax` by `FAERIE_FIRE_DUTY_RESTACK_ALLOWANCE` (5 casts) when the fight's own `computeFaerieFireDuty` result is `onDuty: true`, calibrated to bring the FF-duty judgement distribution closest to the non-FF-duty baseline of any integer allowance tested. Surfaced in `RestackTaxCard` via a visible callout and updated threshold text whenever the allowance is applied.
- **Accidental blooms: cross-druid artifact, not a real effect — no mitigation.** The within-druid paired median delta is exactly 0 across the full paired sample (11 positive, 11 negative, 5 tied, n=27) — a cleaner "no effect" showing than an initial pooled read (which showed a good→fair shift) suggested. The pooled shift was a cross-druid confound (different druids populating each group), not a real per-druid effect.
- **Ending mana %: a real per-druid effect exists, but it isn't drag — no mitigation.** The within-druid paired median delta is a real, majority-direction −9.5 percentage points (17 of 27 negative) — FF-duty fights end with meaningfully less leftover mana than that same druid's own non-FF-duty fights. This doesn't indicate drag: `judgeManaBand`'s bands are non-monotonic (bad only above 70%, hoarding; good sits in the middle, 5–40%), so a downward shift moves fights away from the bad/hoarding band, not toward it — confirmed by the real judgement distribution, where FF-duty's bad rate (7.4%) is actually lower than non-FF-duty's (9.1%).
- **LB3 refresh cadence: confirmed no drag.** FF-duty fights show a better judgement distribution (24% good/39% bad) than non-FF-duty (14% good/48% bad) — if anything the opposite of drag.
- **Consumable throughput: confirmed no difference.** Judgement distributions are essentially identical between groups (63%/24%/13% vs. 66%/22%/12% good/fair/bad).

Story 917 is done: the empirical check found exactly one metric with a real, robust drag (re-stack tax), and that metric's mitigation has been designed, calibrated against the real corpus, and implemented (`src/metrics/restackTax.ts`'s `FAERIE_FIRE_DUTY_RESTACK_ALLOWANCE`), wired consistently into both the single-fight Scorecard and the whole-report dashboard, and surfaced transparently in the UI per this project's principle 3.
