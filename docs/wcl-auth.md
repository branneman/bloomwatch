...
As a stretch check, the report's `masterData.actors` list also resolved correctly and included the target druid (Dassz, actor id 2) by exact name match — confirming actor lookups work the same way. A cast-events query for Dassz against the report's first fight ID returned an empty event list; this is because the naive "pick fights[0]" logic landed on a zero-duration trash/pull segment, not a real boss encounter — not an API or auth problem. Querying `events` for one of the report's actual boss fights (e.g. id 6, "The Lurker Below") would be expected to return real cast data; a future implementation should pick a fight by duration or by boss-encounter type rather than by list position.
