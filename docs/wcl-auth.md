...
As a stretch check, the report's `masterData.actors` list also resolved correctly and included the target druid (Dassz, actor id 2) by exact name match — confirming actor lookups work the same way. The first cast-events attempt picked `fights[0]`, which is a zero-duration trash/pull marker (`startTime` and `endTime` both 760292), and correctly returned an empty event list — not an API or auth problem, just a bad fight pick. Fixed by (a) requesting `startTime`/`endTime` in the fights query and (b) selecting the first fight with `endTime > startTime` instead of list position. Re-tested live against fight id 3 ("Coilfang Frenzy"): the `events(dataType: Casts)` query returned 10 real cast events for Dassz — `cast`/`begincast` entries with real `abilityGameID`s and `targetID`s — confirming the full pipeline (auth → report → actor lookup → fight selection → event stream) works end to end. This satisfies the roadmap's Phase 0 exit criterion: a hardcoded page printing a druid's real cast timeline for one fight.
